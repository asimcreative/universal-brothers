...
- **Addendum (post-launch client correction):** the data model must support PKR/SAR/USD as three genuine currency columns on every price (one package, three currencies — never a duplicate package per currency), a dynamic list of sharing types (the brochure itself prints a 4th type, "Sharing Room," beyond Quad/Triple/Double — never hardcode to three), and Aziziya pricing/accommodation as its own dedicated sub-structure that must never be conflated with the main package price. See `docs/source-documents/HAJJ_BROCHURE_EXTRACTION.md` for the full re-verified brochure extraction this drove, and `HAJJ_PACKAGE_DATA_AUDIT.md` for the resulting per-package verification. The brochure itself remains USD-only — no real PKR/SAR value exists to seed; only the structure is required now.
...
- **Addendum (project owner business confirmation):** the project owner has since confirmed the approved **public-facing** pilgrim-count figure is **"10,000+ Hajis Served"** (superseding the "50,000+" figure recovered from the live-site audit above for display purposes — `site_settings.pilgrims_served` updated accordingly) and the approved **public-facing** awards statistic is **"20+ Awards & Recognitions"**. The 7 real, named award records recovered from the live-site audit (see `AwardSeeder.php`) remain seeded exactly as-is and are explicitly provisional — no award names were invented to reach 20; `industry_awards_count` (the public statistic) is deliberately kept decoupled from the `Award` model's actual row count until the owner supplies the remaining real award names/details.
