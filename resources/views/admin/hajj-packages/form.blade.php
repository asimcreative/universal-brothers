...
@section('content')
    <form method="POST" action="{{ $package->exists ? route('admin.hajj-packages.update', $package) : route('admin.hajj-packages.store') }}" enctype="multipart/form-data">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                <p class="small text-muted">Leave empty if this package has no Package A/B split (e.g. a single accommodation column throughout). Other sections below reference a variant by typing the same Code (e.g. "A") — not by picking from a list — so rows can be reordered freely.</p>
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white fw-semibold">Aziziya</div>
            <div class="card-body row g-3">
                <div class="col-md-3">
            ...
            <div class="card-header bg-white fw-semibold d-flex justify-content-between border-top">
                Aziziya Room / Sharing Options
        ...
        @endphp
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white fw-semibold">Mina</div>
            <div class="card-body row g-3">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white fw-semibold">Arafat</div>
            <div class="card-body row g-3">
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-header bg-white fw-semibold d-flex justify-content-between">
                Main Package Room / Sharing Prices
                <button type="button" class="btn btn-sm btn-outline-primary" data-repeater-add="room-options-container" data-repeater-template="room-option-row-template">+ Add Sharing Option</button>
            </div>
            <div class="card-body">
                <p class="small text-muted">Add as many sharing types as this package actually offers (Quad, Triple, Double, Sharing Room, Twin, Single, ...) — not limited to a fixed list. Leave a currency blank if the brochure gives no value for it (do not guess). Uncheck "Available" for a real "N/A" cell rather than leaving the row out.</p>
                <div class="table-responsive">
                    <table class="table table-sm align-middle">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-sm align-middle">
                        <thead><tr><th>Day #</th><th>Date</th><th>Hijri Label</th><th>City</th><th>Accommodation A</th><th>Accommodation B</th><th>Notes</th><th></th></tr></thead>
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                <p class="small text-muted">Main-package upgrades only (e.g. Kaba view supplement, additional Medinah night) — Aziziya-specific upgrades belong in the Aziziya section above, not here.</p>
                    ...
                    @foreach($mediaRows as $i => $row)
                        <div class="row g-2 align-items-end mb-2 repeater-row border-bottom pb-2">
                            ...
                            <div class="col-md-3">
                                ...
                                @if($row['image_path'])
                                    <img src="{{ Storage::url($row['image_path']) }}" class="mt-1" style="height:40px;" alt="Current media image">
                                @endif
                            </div>
