@php
    ...
    // WhatsApp comes from the office record rather than from Settings, which
    // is why it is not part of the social-links component.
    $ubWhatsapp = $primaryOffice?->whatsapp;
    $ubWhatsappHref = $ubWhatsapp ? 'https://wa.me/'.preg_replace('/[^\d]/', '', $ubWhatsapp) : null;
    ...
    // The service links come from the shared tree so this footer carries
    // everything the Bootstrap pages' footer carried — the whole Hajj, Umrah
    // and Tourism depth, not the five-link summary this one used to show.
    $ubServiceGroups = \App\Support\SiteNavigation::footerServices($navCategories);
...
{{--
    ...
    The class names belong to our own stylesheet, so they are
...
--}}
<footer class="ub-chrome bg-brand-900 text-ivory">
    <div class="mx-auto w-full max-w-[var(--container-site)] px-[1.25rem] sm:px-8 lg:px-10">
        <div class="grid grid-cols-2 gap-x-6 gap-y-10 pt-16 sm:pt-20 lg:grid-cols-3 xl:grid-cols-[1.6fr_1fr_1fr_1fr_0.9fr_1.25fr] xl:gap-8">
            <div class="reveal-on-scroll col-span-2 sm:col-span-1">
                <a class="flex items-center gap-[0.75rem]" aria-label="Universal Brothers" href="{{ route('home') }}">
            ...
            <div class="reveal-on-scroll reveal-delay-1">
                ...
                <ul class="mt-[1.25rem] space-y-px">
                    @foreach($ubQuickLinks as [$ubLabel, $ubUrl])
                        <li>
                            <a class="group flex items-center gap-[0.75rem] py-1.5 text-sm text-on-dark-muted transition-colors duration-300 hover:text-accent-500" href="{{ $ubUrl }}">
            ...
            {{-- One column per service, rather than all three stacked in one.
                 Thirteen links and three labels in a single column made it
                 roughly seven hundred pixels tall while the columns beside it
                 ended after five, which is what left the footer looking
                 bottom-heavy and mostly empty. --}}
            @foreach($ubServiceGroups as $ubIndex => $ubGroup)
                <div class="reveal-on-scroll reveal-delay-{{ min($ubIndex + 2, 6) }}">
                    <h3 class="font-display text-lg font-bold text-on-dark">{{ $ubGroup['title'] }}</h3>
                    <ul class="mt-[1.25rem] space-y-px">
                        @foreach($ubGroup['links'] as [$ubLabel, $ubUrl])
                            <li>
                                <a class="group flex items-center gap-[0.75rem] py-1.5 text-sm text-on-dark-muted transition-colors duration-300 hover:text-accent-500" href="{{ $ubUrl }}">
                                    <span aria-hidden="true" class="size-1.5 shrink-0 rounded-full bg-accent-500 transition-transform duration-300 group-hover:scale-150"></span>
                                    <span class="transition-transform duration-300 group-hover:translate-x-1">{{ $ubLabel }}</span>
                    ...
                    @if($primaryOffice?->phone_primary)
                        <li>
                            <a href="tel:{{ preg_replace('/[^+\d]/', '', $primaryOffice->phone_primary) }}" class="group flex items-start gap-3.5">
                                ...
                                <span class="text-sm leading-relaxed text-on-dark transition-colors duration-300 group-hover:text-ivory">{{ $primaryOffice->phone_primary }}</span>
                            </a>
                        </li>
                    @endif
                    @if($primaryOffice?->email)
                        <li>
                            <a href="mailto:{{ $primaryOffice->email }}" class="group flex items-start gap-3.5">
                                ...
                                <span class="text-sm leading-relaxed text-on-dark transition-colors duration-300 group-hover:text-ivory">{{ $primaryOffice->email }}</span>
                            </a>
                        </li>
                    @endif
                    @if($primaryOffice?->address)
                        <li>
                            <div class="group flex items-start gap-3.5">
                                ...
                                <span class="text-sm leading-relaxed text-on-dark transition-colors duration-300 group-hover:text-ivory">{{ $primaryOffice->address }}</span>
                            </div>
                        </li>
                    @endif
                </ul>
        ...
        <div>
            ...
            {{-- Two rows, not three columns. Social, copyright and policies
                 together came to more than the container holds, so every one
                 of them wrapped. The two link groups share a line; the
                 copyright has the next one to itself. --}}
            <div class="mt-10 border-t border-ivory/10 pt-6 text-sm">
                <div class="flex flex-col items-center gap-[1.25rem] sm:flex-row sm:justify-between">
                    <ul class="-mx-2 flex flex-wrap items-center justify-center sm:justify-start">
                <x-social-links
                    :size="18"
                    class="contents"
                    link-class="flex size-10 items-center justify-center rounded-full text-on-dark-muted transition-all duration-300 hover:-translate-y-0.5 hover:text-accent-500 focus-visible:text-accent-500 focus-visible:outline-none" />

                @if($ubWhatsappHref)
                    <li><a href="{{ $ubWhatsappHref }}" target="_blank" rel="noopener" aria-label="Universal Brothers on WhatsApp" class="flex size-10 items-center justify-center rounded-full text-on-dark-muted transition-all duration-300 hover:-translate-y-0.5 hover:text-accent-500 focus-visible:text-accent-500 focus-visible:outline-none"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M17.47 14.38c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.95 1.17-.17.2-.35.22-.65.07-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.76-1.66-2.06-.17-.3-.02-.46.13-.61.14-.14.3-.35.45-.53.15-.18.2-.3.3-.5.1-.2.05-.38-.02-.53-.08-.15-.67-1.61-.92-2.2-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.8.38-.27.3-1.04 1.02-1.04 2.48s1.07 2.88 1.22 3.08c.15.2 2.1 3.2 5.08 4.49.71.3 1.26.49 1.69.63.71.22 1.36.19 1.87.12.57-.09 1.76-.72 2.01-1.41.25-.7.25-1.29.17-1.41-.07-.13-.27-.2-.57-.35M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.87 9.87 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91C21.96 6.45 17.5 2 12.04 2"></path></svg></a></li>
                @endif
            </ul>

                    <ul class="flex flex-wrap items-center justify-center gap-x-8 gap-y-3">
                    @foreach($ubLegal as [$ubLabel, $ubUrl])
                        <li><a class="text-on-dark-muted transition-colors duration-300 hover:text-accent-500" href="{{ $ubUrl }}">{{ $ubLabel }}</a></li>
                    @endforeach
                </ul>
                </div>

                <p class="mt-[1.25rem] pb-[0.25rem] text-center text-on-dark-muted sm:text-left">&copy; {{ date('Y') }} Universal Brothers (Pvt) Ltd. All rights reserved. A company of {{ \App\Models\SiteSetting::get('parent_group', "Maxim's Group") }}.</p>
            </div>
        </div>
    </div>
</footer>
