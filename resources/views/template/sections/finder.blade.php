            ...
            <div class="mx-auto mt-12 max-w-5xl lg:mt-14">
                <div class="rounded-card border border-border bg-surface-2 p-5 shadow-[0_16px_40px_-32px_rgba(6,20,24,0.6)] sm:p-6 reveal-on-scroll reveal-delay-3">
                    <form method="GET" action="{{ route('packages.category', 'hajj') }}" id="packageFinder" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-end lg:gap-5">
                        ...
                        @php($ubBudgetHint = ['PKR' => 'e.g. 3500000', 'SAR' => 'e.g. 45000', 'USD' => 'e.g. 12000'][\App\Support\Currency::current()] ?? '')
                        <div class="relative">
                            <label for="finder-budget" class="block font-semibold uppercase tracking-[0.14em] mb-1.5 text-[11px] text-on-dark-muted">Budget up to ({{ \App\Support\Currency::current() }})</label>
                            <input type="number" name="price_max" id="finder-budget" min="0" step="500" placeholder="{{ $ubBudgetHint }}" inputmode="numeric" class="w-full rounded-pill border text-left transition-colors duration-200 h-12 px-4 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500 focus-visible:ring-offset-2 focus-visible:ring-offset-background border-border bg-surface text-foreground hover:border-brand-400">
                        </div>
